...
let rec step_thread (t : thread) : thread * action =
  ...
  let vs', es', act =
    ...
    | Plain e', vs ->
      (match e', vs with
      | Unreachable, vs ->
        vs, [Trapping "unreachable executed" @@ e.at], No_action
      ...
      | Block (bt, es'), vs ->
        ...
        vs', [Label (n2, [], (args, List.map plain es')) @@ e.at], No_action

      | Loop (bt, es'), vs ->
        let FuncType (ts1, ts2) = block_type frame.inst bt in
        let n1 = Lib.List32.length ts1 in
        let args, vs' = take n1 vs e.at, drop n1 vs e.at in
        vs', [Label (n1, [e' @@ e.at], (args, List.map plain es')) @@ e.at], No_action

      | If (bt, es1, es2), I32 0l :: vs' ->
        vs', [Plain (Block (bt, es2)) @@ e.at], No_action

      | If (bt, es1, es2), I32 i :: vs' ->
        vs', [Plain (Block (bt, es1)) @@ e.at], No_action

      | Br x, vs ->
        [], [Breaking (x.it, vs) @@ e.at], No_action

      | BrIf x, I32 0l :: vs' ->
        vs', [], No_action

      | BrIf x, I32 i :: vs' ->
        vs', [Plain (Br x) @@ e.at], No_action

      | BrTable (xs, x), I32 i :: vs' when I32.ge_u i (Lib.List32.length xs) ->
        vs', [Plain (Br x) @@ e.at], No_action

      | BrTable (xs, x), I32 i :: vs' ->
        vs', [Plain (Br (Lib.List32.nth xs i)) @@ e.at], No_action

      | Return, vs ->
        [], [Returning vs @@ e.at], No_action

      | Call x, vs ->
        vs, [Invoke (func frame.inst x) @@ e.at], No_action

      | CallIndirect x, I32 i :: vs ->
        let func = func_elem frame.inst (0l @@ e.at) i e.at in
        if type_ frame.inst x <> Func.type_of func then
          vs, [Trapping "indirect call type mismatch" @@ e.at], No_action
        else
          vs, [Invoke func @@ e.at], No_action
      ...
      | GlobalGet x, vs ->
        Global.load (global frame.inst x) :: vs, [], No_action

      | GlobalSet x, v :: vs' ->
        (try Global.store (global frame.inst x) v; vs', [], No_action
        ...
        (try
          ...
          if v = ve then
            if timeout >= 0L && timeout < timeout_epsilon then
              I32 2l :: vs', [], No_action (* Treat as though wait timed out immediately *)
            else
              (* TODO: meaningful timestamp handling *)
              vs', [Suspend (mem, addr, 0.) @@ e.at], (Wait_action (mem, addr, 0.))
          else
            I32 1l :: vs', [], No_action  (* Not equal *)
        with exn -> vs', [Trapping (memory_error e.at exn) @@ e.at], No_action)
      ...
      | MemorySize, vs ->
        let mem = memory frame.inst (0l @@ e.at) in
        I32 (Memory.size mem) :: vs, [], No_action
      ...
      | MemoryGrow, I32 delta :: vs' ->
        ...
        in I32 result :: vs', [], No_action

      | Const v, vs ->
        v.it :: vs, [], No_action

      | Test testop, v :: vs' ->
        (try value_of_bool (Eval_numeric.eval_testop testop v) :: vs', [], No_action
        with exn -> vs', [Trapping (numeric_error e.at exn) @@ e.at], No_action)

      | Compare relop, v2 :: v1 :: vs' ->
        (try value_of_bool (Eval_numeric.eval_relop relop v1 v2) :: vs', [], No_action
        with exn -> vs', [Trapping (numeric_error e.at exn) @@ e.at], No_action)

      | Unary unop, v :: vs' ->
        (try Eval_numeric.eval_unop unop v :: vs', [], No_action
        with exn -> vs', [Trapping (numeric_error e.at exn) @@ e.at], No_action)

      | Binary binop, v2 :: v1 :: vs' ->
        (try Eval_numeric.eval_binop binop v1 v2 :: vs', [], No_action
        with exn -> vs', [Trapping (numeric_error e.at exn) @@ e.at], No_action)

      | Convert cvtop, v :: vs' ->
        (try Eval_numeric.eval_cvtop cvtop v :: vs', [], No_action
        with exn -> vs', [Trapping (numeric_error e.at exn) @@ e.at], No_action)
    ...
    | Label (n, es0, (vs', {it = Trapping msg; at} :: es')), vs ->
      vs, [Trapping msg @@ at], No_action

    | Label (n, es0, (vs', {it = Returning vs0; at} :: es')), vs ->
      vs, [Returning vs0 @@ at], No_action

    | Label (n, es0, (vs', {it = Breaking (0l, vs0); at} :: es')), vs ->
      take n vs0 e.at @ vs, List.map plain es0, No_action

    | Label (n, es0, (vs', {it = Breaking (k, vs0); at} :: es')), vs ->
      vs, [Breaking (Int32.sub k 1l, vs0) @@ at], No_action
    ...
    | Invoke func, vs ->
      ...
      | Func.AstFunc (t, inst', f) ->
        ...
        vs', [Frame (n2, frame', ([], instr')) @@ e.at], No_action

      | Func.HostFunc (t, f) ->
        try List.rev (f (List.rev args)) @ vs', [], No_action

...
let run_action c act : Values.value list option =
  ...
  | Invoke (x_opt, name, args) ->
    ...
    | Some (Instance.ExternFunc f) ->
      ...
      if List.length args <> List.length ins then
...
let run_assertion c ass : assertion option =
  ...
  | AssertMalformed (def, re) ->
    ...
    | exception Decode.Code (_, msg) -> assert_message ass.at "decoding" msg re; None
    | exception Parse.Syntax (_, msg) -> assert_message ass.at "parsing" msg re; None
